...
<div class="flex flex-col gap-5">
    <div class="flex items-center gap-2">
        ...
        <h1 class="text-2xl font-bold">
            {{ isset($news) ? 'Редактировать новость: ' . $news->name : 'Добавить новую новость' }}
        </h1>
    </div>
    ...
    <form action="{{ isset($news) ? route('admin.news.update', $news) : route('admin.news.store') }}" 
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex flex-col gap-6">
            ...
            <div>
                ...
                <textarea name="description" rows="5" class="mt-1 w-full rounded-md border-gray-300" required>{{ old('description', $news->description ?? '') }}</textarea>
            </div>

            @include('admin.partials.image-upload', ['model' => $news ?? null])

            <button type="submit" class="bg-indigo-600 text-white py-3 px-4 rounded-md hover:bg-indigo-700 transition">
                {{ isset($news) ? 'Сохранить изменения' : 'Опубликовать новость' }}
            </button>
        </div>
    </form>
</div>
