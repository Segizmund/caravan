...
<div class="flex flex-col gap-5">
    <form action="{{ route('admin.news.index') }}" method="GET" class="p-5 bg-white rounded-lg shadow-sm border border-gray-100 flex gap-4 items-end">
        <div class="flex-1">
            <label class="block text-xs font-bold text-gray-500 uppercase">Название новости</label>
            <input type="text" name="search" value="{{ $search }}" placeholder="Найти новость..." 
        ...
        <a href="{{ route('admin.news.index') }}" class="text-gray-500 hover:text-gray-700 px-4 py-2 border rounded">Сброс</a>
    </form>
    <div>
    ...
    <div>
        <h2 class="text-xl font-bold">Список всех новостей</h2>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        @forelse($news as $item)
            <div class="bg-white p-5 border rounded shadow relative group">
                ...
                <a href="{{ route('admin.news.edit', $item->id) }}" class="absolute cursor-pointer opacity-0 group-hover:opacity-100 -top-5 right-10 bg-white hover:bg-green-700 p-2 rounded-full border transition duration-300 group/btn">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="group-hover/btn:fill-white transition" viewBox="0 0 16 16">
                ...
                @if($item->main_image)
                    <img src="{{ asset('storage/' . $item->main_image->path) }}" 
                        alt="{{ $item->title }}" 
        ...
        @empty
            <div class="p-10 text-center text-gray-500 col-span-full border-2 border-dashed rounded-lg">
                @if($hasSearchQuery)
                    ...
                    <a href="{{ route('admin.news.index') }}" class="text-indigo-600 hover:underline">Сбросить фильтр</a>
                @else
                    <h3 class="text-lg font-medium">Список новостей пуст</h3>
                    <p>Вы еще не добавили ни одной новости. Нажмите кнопку <a href="{{ route('admin.news.create') }}" class="text-indigo-600 hover:underline">«Добавить новую новость»</a>, чтобы создать первую.</p>
                @endif
            </div>
        @endforelse
    </div>
